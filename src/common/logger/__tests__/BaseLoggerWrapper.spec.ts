...
describe('logger wrapper', () => {
  describe('logger wrapper without mask', () => {
    ...
    describe('Non await asynchronouse function', () => {
      it('should log debug once and error once when promise rejects and return Promise type', async () => {
        const service = wrapLogs({
          getName: () => Promise.reject('Name')
        });
        expect(service.getName() instanceof Promise).toBeTruthy();
        expect(logger.debug).toHaveBeenCalledTimes(1);
        expect(logger.error).toHaveBeenCalledTimes(0);
      });
      it('should log debug once and error once when promise resolves and return Promise type', async () => {
        const service = wrapLogs({
